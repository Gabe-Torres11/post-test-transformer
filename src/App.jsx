...
/**
 * Walk through all non-metadata columns and separate them into:
 *   answerCols — columns whose values are single letters A–E (student choices)
 *   scoreCols  — columns whose values are 0 or 1 (correct/incorrect)
 *
 * Canvas exports interleave these in order, so answerCols[i] and scoreCols[i]
...
export default function App() {
  ...
  function autoDetect(courseNum, tidsMap) {
    if (courseNum) {
      // Pre-fill the course number so the user only has to type the section
      setClassNumber(prev => {
        if (prev && prev !== courseNum + '-') return prev; // don't overwrite if user typed something
        return courseNum + '-';
      });
    }
  ...
  function handleTestIdsFile(file) {
    ...
    reader.onload = (e) => {
      ...
      // Scan the first few rows for a header containing "COURSE" and "TEST" + "ID"
      const map = {};
